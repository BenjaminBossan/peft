...
"""Main entry point for the FAAST vs gradient-training image generation experiments.

Copied from method_comparison/image-gen (which is in turn based on
https://github.com/huggingface/diffusers/blob/bbbcdd87bd9d960fa372663a50b9edbdcb1391c6/examples/dreambooth/train_dreambooth_lora_flux2_klein.py).

The difference to the image-gen benchmark: experiments whose adapter config is a FAAST config are not trained with an
optimizer. Instead, the same data pipeline (latents, prompts, noise and timestep sampling) is used to run forward-only
passes during which the FAAST layer on the transformer's output projection accumulates the statistics for its
closed-form fast weight solve; the regression targets are the packed flow matching targets (noise - latents). All
other experiments (e.g. LoRA) are trained exactly like in the image-gen benchmark, so the results are directly
comparable.
"""
...
def train(
    *,
    ...
    if is_faast:
        # FAAST learns forward-only, there is nothing for an optimizer to do
        optimizer, lr_scheduler = None, DummyScheduler(lr=0.0)
        transformer.eval()
    else:
    ...
    try:
        ...
        for step in pbar:
            ...
            if is_faast:
                # forward-only FAAST learning: the layer wrapping the output projection accumulates the statistics
                # for the closed-form solve; the regression targets are the packed flow matching targets. The solve
                # itself only happens before evaluations, as it is not free.
                packed_target = pipeline._pack_latents(target)
                transformer.base_model.begin_learn(values=packed_target)
                try:
                    with torch.no_grad():
                    ...
                    transformer.base_model.end_learn(solve=False)
                with torch.no_grad():
                    ...
                    # this is the loss of the frozen base prediction (the fast weights are inactive while learning),
            ...
            if step % train_config.eval_steps == 0:
                ...
                if is_faast:
                    transformer.base_model.solve_fast_weights()
                transformer.eval()
        ...
        if is_faast:
            transformer.base_model.solve_fast_weights()
            for faast_layer in transformer.base_model._faast_layers():
                for adapter_name, num_tokens in faast_layer.faast_num_tokens.items():
                    print_verbose(f"FAAST learned pairs ({adapter_name}): {int(num_tokens)}")
                    ratio = faast_layer.faast_fit_ratio.get(adapter_name)
                    if ratio is not None:
                        print_verbose(f"FAAST explained target energy ({adapter_name}): {ratio:.4f}")
        transformer.eval()
...
def main(*, path_experiment: str, experiment_name: str, clean: bool, bucket_name: Optional[str]) -> None:
    ...
    train_result = train(
        ...
        is_faast=peft_config is not None and peft_config.peft_type == "FAAST",
